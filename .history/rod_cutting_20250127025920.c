...
void output_cuts(int *lengths, int *values, int count, int rodLength, int totalValue) {
    for (int i = 0; i < count - 1; i++) {
...
int main(int argc, char *argv[]) {
    ...
    printf("Enter piece prices in the format <length>, <value> (press Ctrl+Z "
           "to end input):\n");
    ...
    if (count == 0) {
        fprintf(stderr, "Must have at least one line of input formatted "
                        "<length>, <value>\n");
        return 1;
    }
    ...
    cut(lengths, values, count, rodLength, totalValue);
}